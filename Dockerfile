...
CMD cd src/tenable_ibm_cp4s && python3 cli.py -accountId=${ACCOUNT_ID} -clientID=${CLIENT_ID} -clientSecret=${CLIENT_SECRET} -region=${REGION} -url=${CAR_SERVICE_URL} -api_key=${CAR_SERVICE_KEY} -password=${CAR_SERVICE_PASSWORD} -source=${SOURCE} -working_dir=/tmp
